    ...
    <main style={{ maxWidth: 720, margin: '0 auto', padding: '24px 20px', minHeight: '100vh' }}>
      ...
      {tab === 'analisis' ? (
        <>
          ...
          {step === 'resultado' && resultado && <Resultado grading={resultado} onNuevo={() => { setStep(1); setResultado(null); setForm({ habilidades_tecnicas: [], certificaciones: [], idiomas: [], soft_skills: { comunicacion: 5, negociacion: 5, empatia: 5, resolucion_conflictos: 5 }, personas_a_cargo: 0 }); }} />}
        </>
      ) : (
        <div>
          {historial.length === 0 ? (
