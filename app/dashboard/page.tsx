...
// PULSO Salary Wizard - Codify Analytics
...
export default function Dashboard() {
  ...
  const downloadPDF = async () => {
    if (!analysisResults || !user) {
      alert('Error: faltan datos del análisis');
      return;
    }

    try {
      await generateCertificatePDF(analysisResults, user.email);
    } catch (error) {
      ...
      <div style={{ minHeight: '100vh', background: '#1a1a2e', padding: '40px 20px' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          ...
          <div style={{ textAlign: 'center', display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={downloadPDF} style={{ padding: '12px 32px', backgroundColor: '#BF057D', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' }}>📥 Descargar Certificado PDF</button>
